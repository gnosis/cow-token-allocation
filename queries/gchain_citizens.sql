    ...
    select claimant,
           ...
           case
               when claimable_gno_option is not null
                   then gno_option_claimed / claimable_gno_option end   as gno_option_exercised,
           case
               when claimable_user_option is not null
                   then user_option_claimed / claimable_user_option end as user_option_exercised,
           case
               when greatest(gno_option_claimed, user_option_claimed) =
                    gno_option_claimed then 'GNO'
               when greatest(gno_option_claimed, user_option_claimed) =
                    user_option_claimed then 'xDAI'
