...
investment_claims as (
    select concat('0x', encode(claimant, 'hex'))                     as claimant,
           sum(case
                   when "claimType" = 1 then "claimedAmount"
                   else 0
               end)                                                  as gno_option_claimed,
           sum(case
                   when "claimType" = 2 then "claimedAmount"
                   else 0
               end)                                                  as user_option_claimed,
           sum(case
                   when "claimType" = 3 then "claimedAmount"
                   else 0
               end)                                                  as investor_option_claimed,
